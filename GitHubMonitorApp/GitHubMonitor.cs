...
namespace GitHubMonitorApp
{
    public static class GitHubMonitor
    {
        ...
        public static async Task<IActionResult> Run(
            //removed get, only want to post
            [HttpTrigger(AuthorizationLevel.Function, "post", Route = null)] HttpRequest req,
            ILogger log)
        {
            log.LogInformation("Our GitHub Monitor processed an action.");

            //reads the whole body -- this is where json data may be
            string requestBody = await new StreamReader(req.Body).ReadToEndAsync();

            //reading from a json file iot access data
            // deserialize as a rootobject from PASTE SPECIAL JSON CLASSES
            var data = JsonConvert.DeserializeObject<Rootobject>(requestBody);
            /* 
                ex: {firstName: "Austin", lastName: "Williams"}
                data.firstName = "Austin";
                data.lastName = "Williams";
            */

            log.LogInformation(requestBody); //logging the requestbody, could be written out to sql
...
//pasted json as classes from Monitor in Azure Function GUI
